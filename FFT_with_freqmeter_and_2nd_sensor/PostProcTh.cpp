...
void __fastcall TPostProcThread::CalibrateInAccel(channels *ch)
{
	...
	ch->D = (ch->D - DZeroOffset)/DSensetivity;
	ch->E = (ch->E - EZeroOffset)/ESensetivity;
	ch->F = (ch->F - FZeroOffset)/FSensetivity;
...
void TPostProcThread::OpenFile(AnsiString S)
{
	...
	pFile << setiosflags(ios_base::scientific);
...
void TPostProcThread::setFileName(AnsiString s)
{
	filename = s;
}
void __fastcall TPostProcThread::ThUpdate (tagMSG &msg)
{
	if ( msg.message == F_OPEN )
		OpenFile(filename);

	if ( msg.message == F_CLOSE )
		closeFile();
