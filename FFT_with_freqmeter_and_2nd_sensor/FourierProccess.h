...
#define CxVector vector<FFT::Complex>
...
protected:
	...
	void __fastcall getMinMax(const CxVector& arr, double &max, double &min, int &maxindex, int &minindex);
	void __fastcall getMinMax(const CxVector& arr, double &max, double &min, int &maxindex, int &minindex, int limit);

	void __fastcall getMax(const CxVector& arr, double &max, int &maxindex, int limit);

	void __fastcall FFTPostProc( CxVector& bufA, CxVector& bufB, CxVector& bufD,CxVector& bufE, CxVector& bufF);
	void __fastcall CorrectSpectrum( CxVector& bufA, CxVector& bufB, CxVector& bufD,CxVector& bufE, CxVector& bufF);

	vector<FFT::Complex> A_FFT,iA_FFT;
	vector<FFT::Complex> B_FFT,iB_FFT;
	vector<FFT::Complex> D_FFT,iD_FFT;
	vector<FFT::Complex> E_FFT,iE_FFT;
	vector<FFT::Complex> F_FFT,iF_FFT;
...
public:
	...
	__property double freq = {read = m_freq} ;
