...
void __fastcall TForm1::FCalc(TObject *Sender)
{
	...
	Edit1->Text = FloatToStr(RoundTo(myTh->fourierTh->freq, -4));
	Edit2->Text = FloatToStr(RoundTo(myTh->fourierTh->freq/60.0, -4));
	ProgressBar1->Position = myTh->fourierTh->freq;
...
void __fastcall TForm1::Button1Click(TObject *Sender)
{
	if (OpenDialog1->Execute())
	{
		...
		myTh->postTh->setFileName(S);
		PostThreadMessage(myTh->postTh->ThreadID, F_OPEN, 0 , 0);
		Edit3->Text = S;
	}
...
void __fastcall TForm1::Button3Click(TObject *Sender)
{
	...
	{
		PostThreadMessage(myTh->postTh->ThreadID, F_CLOSE, 0 , 0);
	}
