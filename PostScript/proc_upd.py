...
path = "./Poz_mash_v_klasse_disbalance3/"
#path = "./"
...
ADA_BASE_FREQ_C = 3.635e5
dt = 1.0/(ADA_BASE_FREQ_C/7.0/2.0)
...
def preproc(res):
        x1 = np.zeros(res[:,2].size)
        x2 = np.zeros(res[:,2].size)
        i = 0
        k = 0
        res_size = res[:,2].size - 1
        while (i < res_size):
            if (res[i,2] > res[i+1,2]):
                j = i + 1
                while ((j < res_size) and (res[i,2] > res[j,2])):
                    j += 1
