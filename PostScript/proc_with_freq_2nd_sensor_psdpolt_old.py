...
def plot2(x, y, minn, maxn, filename, col=0):
    ...
    if convertToDisplacement:
        ...
        plt.title(u'СКЗ = ' + u"{0:.2f}".format(ny.std()) + u' м/с²')
    plt.plot(nx, ny, lw=2, color=colors[col])
    plt.xlabel(u't, с')
    if convertToDisplacement:
        plt.ylabel(u'u, м')
    else:
    ...
    # plt.semilogx(ff, psd_period, lw=2, color = colors[col] )
    plt.plot(ff, psd_period, lw=2, color=colors[col])
    if convertToDisplacement:
        plt.ylabel(u'S, м²/Гц')
    else:
        plt.ylabel(u'S, (м/с²)²/Гц')
    plt.xlabel(u'f, Гц')
    plt.xlim(1, 10)
...
def loadfile(filename):
    ...
    num_of_counts = int(res[:, 0].size / 4.0)
    ...
    for i in range(num_of_signals - 1):
        ...
        #        plt.plot(freq, corr)
        #        plt.show()
        plot2(t, amp, minn, maxn, path + "out/" + filename + "--signal+psd--" + str(i) + "---.png", i)
        #        exit(1)
